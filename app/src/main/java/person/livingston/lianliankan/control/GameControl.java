...
public class GameControl implements IGameControl {
     ...
     * @param config 游戏配置对象
     */
    public GameControl(GameConf config) {
        // 将游戏的配置对象设置本类中
        this.config = config;
    }
    ...
    public void start() {
        ...
        // 初始化Animal[][]数组
        this.animals = board.create(config);
    }
    ...
    public boolean hasAnimals() {
        ...
        for (int i = 0; i < animals.length; i++) {
            for (int j = 0; j < animals[i].length; j++) {
                // 只要任意一个数组元素不为null，也就是还剩有非空的Animal对象
                if (animals[i][j] != null) {
                    return true;
                }
            }
    ...
    /**
     * 工具方法：计算相对于animal[][]数组的第一维 或第二维的索引值
     *
     * @param relative 座标
    ...
    @Override
    public Animal findAnimal(float touchX, float touchY) {
        /*
         * 由于在创建Animal对象的时候, 将每个Animal的开始座标加了
        ...
        // 返回Animal[][]数组的指定元素
        return this.animals[indexX][indexY];
    }

    @Override
    public LinkInfo link(Animal animal1, Animal animal2) {
        // 两个animal是同一个, 即选中了同一个方块, 返回null
        if (animal1.equals(animal2))
            return null;
        // 如果animal1的图片与animal2的图片不相同, 则返回null
        if (!animal1.isSameImage(animal2))
            return null;
        // 如果animal2在animal1的左边, 则需要重新执行本方法, 两个参数互换
        if (animal2.getIndexX() < animal1.getIndexX())
            return link(animal2, animal1);
        // 获取animal1的中心点
        Point p1Point = animal1.getCenter();
        // 获取animal2的中心点
        Point p2Point = animal2.getCenter();
        // 情况1：如果两个animal在同一行，并且可以直接相连
        if (animal1.getIndexY() == animal2.getIndexY()) {
            // 它们在同一行并可以相连
            if (!isXBlock(p1Point, p2Point, GameConf.ANIMAL_WIDTH)) {
                // 它们之间没有真接障碍, 没有转折点
                return new LinkInfo(p1Point, p2Point);
            }
        }
        // 情况2：如果两个animal在同一列，并且可以直接相连
        if (animal1.getIndexX() == animal2.getIndexX()) {
            if (!isYBlock(p1Point, p2Point, GameConf.ANIMAL_HEIGHT)) {
                // 它们之间没有真接障碍, 没有转折点
                return new LinkInfo(p1Point, p2Point);
            }
        }
        ...
        Map<Point, Point> turns = getLinkPoints(p1Point, p2Point,
                GameConf.ANIMAL_WIDTH, GameConf.ANIMAL_WIDTH);
        // 它们之间有转折点
        if (turns.size() != 0) {
            // 获取p1和p2之间最短的连接信息
    ...
    private Map<Point, Point> getLinkPoints(Point point1, Point point2,
                                            int animalWidth, int animalHeight) {
        Map<Point, Point> result = new HashMap<>();

        // 获取以point1为中心的向上的通道
        List<Point> p1UpChanel = getUpChanel(point1, point2.y, animalHeight);
        // 获取以point1为中心的向右的通道
        List<Point> p1RightChanel = getRightChanel(point1, point2.x, animalWidth);
        // 获取以point1为中心的向下的通道
        List<Point> p1DownChanel = getDownChanel(point1, point2.y, animalHeight);
        // 获取以point2为中心的向下的通道
        List<Point> p2DownChanel = getDownChanel(point2, point1.y, animalHeight);
        // 获取以point2为中心的向左的通道
        List<Point> p2LeftChanel = getLeftChanel(point2, point1.x, animalWidth);
        // 获取以point2为中心的向上的通道
        List<Point> p2UpChanel = getUpChanel(point2, point1.y, animalHeight);

        // 获取Board的最大高度
        int heightMax = (this.config.getYSize() + 1) * animalHeight
        ...
        if (isLeftUp(point1, point2) || isLeftDown(point1, point2)) {
            // 参数换位, 调用本方法
            return getLinkPoints(point2, point1, animalWidth, animalHeight);
        }
        // 情况1：如果p1、p2位于同一行而不能直接相连，需要两个转折点，可以在上面相连也可以在下面相连
        if (point1.y == point2.y) {// 在同一行
            // 第1步: 向上遍历
            // 以p1的中心点向上遍历获取点集合
            p1UpChanel = getUpChanel(point1, 0, animalHeight);
            // 以p2的中心点向上遍历获取点集合
            p2UpChanel = getUpChanel(point2, 0, animalHeight);
            // 如果两个集合向上中有Y坐标相同，即在同一行，且之间没有障碍物
            Map<Point, Point> upLinkPoints = getXLinkPoints(p1UpChanel,
                    p2UpChanel, animalHeight);

            // 第2步: 向下遍历, 不超过Board(有方块的地方)的边框
            // 以p1中心点向下遍历获取点集合
            p1DownChanel = getDownChanel(point1, heightMax, animalHeight);
            // 以p2中心点向下遍历获取点集合
            p2DownChanel = getDownChanel(point2, heightMax, animalHeight);
            // 如果两个集合向上中有Y坐标相同，即在同一行，且之间没有障碍物
            Map<Point, Point> downLinkPoints = getXLinkPoints(p1DownChanel,
                    p2DownChanel, animalHeight);
        ...
        if (isRightUp(point1, point2)) {
            ...
            Map<Point, Point> rightRightLinkPoints = getYLinkPoints(
                    p1RightChanel, p2RightChanel, animalHeight);
            /**********************************************************/
            // 获取以p1为中心的向左通道
            List<Point> p1LeftChanel = getLeftChanel(point1, 0, animalWidth);
            // 获取以p2为中心的向左通道
    ...
    private Map<Point, Point> getYLinkPoints(List<Point> p1Chanel,
                                             List<Point> p2Chanel, int animalHeight) {
        Map<Point, Point> result = new HashMap<>();
        for (int i = 0; i < p1Chanel.size(); i++) {
            ...
            for (int j = 0; j < p2Chanel.size(); j++) {
                ...
                if (temp1.x == temp2.x) {
                    // 没有障碍, 放到map中去
                    if (!isYBlock(temp1, temp2, animalHeight)) {
    ...
    private Map<Point, Point> getXLinkPoints(List<Point> p1Chanel,
                                             List<Point> p2Chanel, int animalWidth) {
        Map<Point, Point> result = new HashMap<>();
        for (int i = 0; i < p1Chanel.size(); i++) {
            // 从第一通道中取一个点
            Point temp1 = p1Chanel.get(i);
            ...
            for (int j = 0; j < p2Chanel.size(); j++) {
                ...
                if (temp1.y == temp2.y) {
                    if (!isXBlock(temp1, temp2, animalWidth)) {
                        // 没有障碍则直接加到结果的map中
                        result.put(temp1, temp2);
                    }
                }
     ...
     */
    private Point getCornerPoint(Point point1, Point point2, int animalWidth,
                                 int animalHeight) {
        // 先判断这两个点的位置关系, 如果point2在point1的左上角或者 point2在point1的左下角
        if (isLeftUp(point1, point2) || isLeftDown(point1, point2)) {
            // 参数换位, 重新调用本方法
            return getCornerPoint(point2, point1, animalWidth, animalHeight);
        }
        // 获取p1向右的通道
        List<Point> point1RightChanel = getRightChanel(point1, point2.x,
                animalWidth);
        // 获取p1向上的通道
        List<Point> point1UpChanel = getUpChanel(point1, point2.y, animalHeight);
        // 获取p1向下的通道
        ...
        List<Point> point2LeftChanel = getLeftChanel(point2, point1.x,
                animalWidth);
        // 获取p2向上的通道
        List<Point> point2UpChanel = getUpChanel(point2, point1.y, animalHeight);
        // 如果point2在point1的右上角
        if (isRightUp(point1, point2)) {
            // 获取p1向右和p2向下的交点
     ...
     * @param p1
     * @param p2
     * @param animalWidth 连连看的每个方块的图片的宽
     * @return 两个animal之间有障碍返回true，否则返回false
     */
    private boolean isXBlock(Point p1, Point p2, int animalWidth) {
        if (p2.x < p1.x) {
            // 如果p2在p1左边, 调换参数位置调用本方法
            return isXBlock(p2, p1, animalWidth);
        }
        for (int i = p1.x + animalWidth; i < p2.x; i = i + animalWidth) {
            if (hasAnimal(i, p1.y)) {// 有障碍
                return true;
            }
        }
        return false;
     ...
     * @param p1
     * @param p2
     * @param animalHeight 连连看的每个方块的图片的高
     * @return 两个animal之间有障碍返回true，否则返回false
     */
    private boolean isYBlock(Point p1, Point p2, int animalHeight) {
        if (p2.y < p1.y) {
            // 如果p2在p1的上面, 调换参数位置重新调用本方法
            return isYBlock(p2, p1, animalHeight);
        }
        for (int i = p1.y + animalHeight; i < p2.y; i = i + animalHeight) {
            if (hasAnimal(p1.x, i)) {
                // 有障碍
                return true;
            }
    ...
    /**
     * 判断GamePanel中的x, y座标中是否有Animal对象
     *
     * @param x
     * @param y
     * @return true 表示有该座标有animal对象 false 表示没有
     */
    private boolean hasAnimal(int x, int y) {
        if (findAnimal(x, y) == null)
            return false;
        return true;
    }
     ...
     *
     * @param p
     * @param animalWidth animal图片的宽
     * @param min         向左遍历时最小的界限
     * @return 给定Point左边的通道
     */
    private List<Point> getLeftChanel(Point p, int min, int animalWidth) {
        List<Point> result = new ArrayList<>();
        // 获取向左通道, 由一个点向左遍历, 步长为Animal图片的宽
        for (int i = p.x - animalWidth; i >= min; i = i - animalWidth) {
            // 遇到障碍, 表示通道已经到尽头, 直接返回
            if (hasAnimal(i, p.y)) {
                return result;
            }
     ...
     * @return 给定Point右边的通道
     */
    private List<Point> getRightChanel(Point p, int max, int animalWidth) {
        List<Point> result = new ArrayList<>();
        // 获取向右通道, 由一个点向右遍历, 步长为Animal图片的宽
        for (int i = p.x + animalWidth; i <= max; i = i + animalWidth) {
            // 遇到障碍, 表示通道已经到尽头, 直接返回
            if (hasAnimal(i, p.y)) {
                return result;
            }
     ...
     * @return 给定Point上面的通道
     */
    private List<Point> getUpChanel(Point p, int min, int animalHeight) {
        List<Point> result = new ArrayList<>();
        // 获取向上通道, 由一个点向右遍历, 步长为Animal图片的高
        for (int i = p.y - animalHeight; i >= min; i = i - animalHeight) {
            // 遇到障碍, 表示通道已经到尽头, 直接返回
            if (hasAnimal(p.x, i)) {
                // 如果遇到障碍, 直接返回
                return result;
     ...
     * @return 给定Point下面的通道
     */
    private List<Point> getDownChanel(Point p, int max, int animalHeight) {
        List<Point> result = new ArrayList<>();
        // 获取向下通道, 由一个点向右遍历, 步长为Animal图片的高
        for (int i = p.y + animalHeight; i <= max; i = i + animalHeight) {
            // 遇到障碍, 表示通道已经到尽头, 直接返回
            if (hasAnimal(p.x, i)) {
                // 如果遇到障碍, 直接返回
                return result;
